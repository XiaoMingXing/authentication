...
import {CacheSessionStorage} from "./cache-session-storage.service";
import {CacheLocalStorage} from "./cache-local-storage.service";
import {StorageValueInterface} from "./cache-storage-value.interface";

const CACHE_PREFIX = 'CacheService';

const DEFAULT_STORAGE = CacheStorageEnum.SESSION_STORAGE;
const DEFAULT_ENABLED_STORAGE = CacheStorageEnum.LOCAL_STORAGE;
...
export class CacheService {
   ...
   */
  public useStorage(type: CacheStorageEnum) {
   ...
   */
  private _initStorage(type: CacheStorageEnum) {
    let storage: CacheStorageAbstract;
    switch (type) {
      case CacheStorageEnum.SESSION_STORAGE:
        storage = new CacheSessionStorage();
        break;
      case CacheStorageEnum.LOCAL_STORAGE:
